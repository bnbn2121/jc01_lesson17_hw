...
public class CoffeeMachine {
	...
	private Map<Drink, Integer> history;
	...
	public void setHistory(Map<Drink, Integer> history) {
		this.history = history;
	}

	public Map<Drink, Integer> getHistory() {
		return history;
	}

	public void doDrink(Drink drink, Portion portion, int sugarCount) {
		int sugarOnePortion = 5;
		if (checkIngridients(drink, portion) & isEnoughIngridient(Ingridient.SUGAR, sugarCount * sugarOnePortion)) {
			int multiply = switch (portion) {
			case ONE -> 1;
			case DOUBLE -> 2;
			};
			log.append("Начинаем приготовление напитка\n");
			for (Ingridient x : drink.getCompound().keySet()) {
				...
				spendIngridient(Ingridient.SUGAR, sugarCount * sugarOnePortion);
			}
			history.put(drink, history.get(drink) + 1);
			log.append(Status.DONE_DRINK.getMessage());
		} else {
			log.append(Status.ERROR_DRINK.getMessage());
		}
	}

	private boolean checkIngridients(Drink drink, Portion portion) {
		int multiply = switch (portion) {
		case ONE -> 1;
		case DOUBLE -> 2;
		};
